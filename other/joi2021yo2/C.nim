...
when isMainModule:
  var N, D, K: int
  (N, D, K) = inputInts()
  var PS = newSeqWith(N, (let ps = inputInts(); (p:ps[0] - 1, s:ps[1]))).sortedByIt(it.s)
  
  var dp: array[200001, array[2, int]]
  for i, (pi, si) in PS:
    dp[i + 1] = dp[i]
    ...
    while ng - ok > 1:
      let j = (ok + ng) div 2
      if PS[j - 1].s + 1 + D + dp[j][1 - pi]*K <= si:
        ok = j
      else:
        ng = j
    dp[i + 1][pi].chmax(dp[ok][1 - pi])
    dp[i + 1][pi].inc
  echo max(dp[N])
