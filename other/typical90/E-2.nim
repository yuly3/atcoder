...
proc matMul*(A, B: seq[seq[int]], MOD: int): seq[seq[int]] =
  let
    N0 = A.len
    N1 = B[0].len
    N2 = A[0].len
...
proc matPow*(A: seq[seq[int]], K, MOD: int): seq[seq[int]] =
  let N = len(A)
  var
    A = A
    K = K
...
when isMainModule:
  ...
  var mat = newSeqWith(B, newSeq[int](B))
  for i in 0..<B:
    for ci in c:
      mat[i][(10*i + ci) mod B] += 1
  
  let ans = matPow(mat, N, MOD)[0][0]
  echo ans
