...
when isMainModule:
  ...
  proc f(items: seq[int]): seq[seq[int]] =
    let n = items.len
    result = newSeqWith(n + 1, newSeq[int]())
    var stack = newSeq[(int, int, int)]()
    stack.add((0, 0, 0))
    while stack.len > 0:
      let (cur, cnt, v) = stack.pop
      if cnt > K:
        continue
      if cur == n:
        result[cnt].add(v)
        continue
      stack.add((cur + 1, cnt + 1, v + items[cur]))
      stack.add((cur + 1, cnt, v))
  
  var
    first = f(A[0..<N div 2])
    latter = f(A[N div 2..<N])
  for i in 1..<latter.len:
    latter[i].sort()
  
  var ans = 0
  for i in 0..min(K, first.len - 1):
    if i + latter.len <= K:
      continue
    for v in first[i]:
      ans += upperBound(latter[K - i], P - v)
  echo ans
