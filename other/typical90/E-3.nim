...
when isMainModule:
  ...
  var pow10: array[64, int]
  for i in 0..<62:
    pow10[i] = modPow(10, 2^i, B)
  
  var dp: array[64, array[1000, int]]
  for ci in c:
    inc dp[0][ci mod B]
  ...
  for i in 0..<61:
    for j in 0..<B:
      for k in 0..<B:
        let n = (pow10[i]*j + k) mod B
        dp[i + 1][n] += dp[i][j]*dp[i][k]
        dp[i + 1][n] %= MOD
  
  var dp2: array[64, array[1000, int]]
  dp2[0][0] = 1
  for i in 0..<61:
    if bitand(N shr i, 1) == 1:
      for j in 0..<B:
        for k in 0..<B:
          let n = (pow10[i]*j + k) mod B
          dp2[i + 1][n] += dp2[i][j]*dp[i][k]
          dp2[i + 1][n] %= MOD
    else:
      for j in 0..<B:
        dp2[i + 1][j] = dp2[i][j]
