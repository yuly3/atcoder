...
when isMainModule:
  ...
  proc f(cnt, prev: int, strnum: string) =
    if cnt == 0:
      var pd = 1
      for i in 0..<strnum.len:
        pd *= strnum[i..i].parseInt()
      pd += B
      if pd <= N:
        var strpd = sorted($pd, order=Descending)
        if strpd == strnum:
          ans.inc
      return
    for i in countdown(prev, 1):
      f(cnt - 1, i, strnum & $char('0'.int + i))
  
  for i in 1..strN.len:
    f(i, 9, "")
  
  if B <= N and "0" in strB:
    ans.inc
  echo ans
