...
when isMainModule:
  var N, Q: int
  (N, Q) = inputInts()
  var L, R = newSeq[int](N - 1)
  for i in 0..<N - 1:
    (L[i], R[i]) = inputInts()
  var
    ai, bi: int
    AB = collect(newSeq):
      for _ in 0..<Q: (ai, bi) = inputInts(); (ai, bi)
  
  var events: seq[(int, int, int, int)]
  for i, li in L:
  ...
  var
    segTree = initSegTree(newSeq[bool](N + 1), (a: bool, b: bool) => a and b, () => true)
    ans = newSeq[int](Q)
  for (age, cmd, pos, idx) in events:
    if cmd == 2:
      let
        rightEnd = segTree.max_right(pos, (t: bool) => t)
        leftEnd = segTree.min_left(pos, (t: bool) => t)
      ans[idx] = rightEnd - leftEnd + 1
    else:
      segTree[pos] = cmd == 1
  echo ans.join("\n")
