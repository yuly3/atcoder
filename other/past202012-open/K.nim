...
when isMainModule:
  var S = newSeqWith(4, input())

  var dp: array[1 shl 16, float]
  for bits in 1..<1 shl 16:
    dp[bits] = 1e9
    for ty in 0..<4:
      for tx in 0..<4:
        var
        ...
        for (dy, dx) in [(0, 0), (-1, 0), (0, 1), (1, 0), (0, -1)]:
          let
            (y, x) = (ty + dy, tx + dx)
            n = 1 shl (4*y + x)
          if y in 0..<4 and x in 0..<4 and bitand(bits, n) > 0:
            inc cnt
            ev += dp[bitxor(bits, n)]
        if cnt != 0:
          dp[bits].chmin(ev / cnt.float)
  
  var bitsS = 0
  for i in 0..<4:
    for j in 0..<4:
      if S[i][j] == '#':
        bitsS = bitor(bitsS, 1 shl (4*i + j))
  echo dp[bitsS]
