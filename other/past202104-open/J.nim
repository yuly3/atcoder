...
when isMainModule:
  ...
  (N, C) = inputInts()
  var xi, yi: float
  let XY = collect(newSeq):
    for _ in 0..<N: (xi, yi) = inputs().map(parseFloat); (xi, yi)
  
  proc f(p: float): float =
    for (xi, yi) in XY:
      result += (p - xi)^2 + (C.float - yi)^2
  
  var (leftest, rightest) = (-1e5, 1e5)
  for _ in 0..<200:
    let
      left = (2*leftest + rightest) / 3.0
      right = (leftest + 2*rightest) / 3.0
    if f(right) < f(left):
      leftest = left
    else:
      rightest = right
