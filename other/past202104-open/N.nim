...
when isMainModule:
  ...
  var
    ai, bi: int
    ab = collect(newSeq):
      for _ in 0..<N: (ai, bi) = inputInts(); (ai, bi)
  
  ab.sort((x, y: (int, int)) => (if x[0].float / x[1].float < y[0].float / y[1].float: 1 else: -1))
  var dp: array[101, array[100001, int]]
  for i, (ai, bi) in ab:
    for j in 0..H:
      dp[i + 1][j].chmax(dp[i][j])
      dp[i + 1][max(j - bi, 0)].chmax(dp[i][j] + j*ai)
  echo max(dp[N])
