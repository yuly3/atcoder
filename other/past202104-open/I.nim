...
when isMainModule:
  ...
  var dp: array[105, array[105, array[210, int]]]
  for i in 0..<H:
    for j in 0..<W:
      for k in 0..<H + W:
        dp[i + 1][j][k].chmax(dp[i][j][k])
        dp[i + 1][j][k + 1].chmax(dp[i][j][k] + A[i][j])
        dp[i][j + 1][k].chmax(dp[i][j][k])
        dp[i][j + 1][k + 1].chmax(dp[i][j][k] + A[i][j])
  
  let ans = collect(newSeq):
    for i in 1..<H + W: dp[H - 1][W][i]
  echo ans.join("\n")
