...
proc rotate(src: seq[seq[char]]): seq[seq[char]] =
  let n = len(src)
  result = newSeqWith(n, newSeq[char](n))
  for i in 0..<n:
    for j in 0..<n:
      result[j][n - i - 1] = src[i][j]

when isMainModule:
  var
    N = inputInt()
    S, T = newSeqWith(N, input().toSeq())
  
  var ans = N^2
  for rot in 0..3:
    var cnt = if rot != 3: rot else: 1
    for i in 0..<N:
      for j in 0..<N:
        if S[i][j] != T[i][j]:
          cnt.inc
