...
when isMainModule:
  ...
  var
    cost: array[31, array[31, array[901, int]]]
    hque = initHeapQueue[tuple[c, d, y, x: int]]()
  const INF = 10^18
  for i in 0..<H:
    for j in 0..<W:
      for k in 0..<H*W:
  ...
  while hque.len > 0:
    ...
    for (dy, dx) in [(-1, 0), (0, 1), (1, 0), (0, -1)]:
      ...
      if 0 <= ny and ny < H and 0 <= nx and nx < W:
        let nc = cc + 2*cd*A[ny][nx] + A[ny][nx]
        if cost[ny][nx][cd + 1] <= nc:
          continue
        cost[ny][nx][cd + 1] = nc
        hque.push((nc, cd + 1, ny, nx))
