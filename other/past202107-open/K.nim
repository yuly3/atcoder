...
when isMainModule:
  ...
  var
    A = inputInts()
    graph: array[100001, seq[(int, int)]]
    ui, vi, ti: int
  for _ in 0..<M:
    (ui, vi, ti) = inputInts()
    graph[ui - 1].add((vi - 1, ti))
    graph[vi - 1].add((ui - 1, ti))
  
  var
    hque = initHeapQueue[(int, int, int)]()
    dist, sati: array[100001, int]
  dist.fill(10^18)
  dist[0] = 0
  sati[0] = -A[0]
  hque.push((0, -A[0], 0))
  while hque.len > 0:
    let (cd, cs, cur) = hque.pop()
    if dist[cur] < cd:
      continue
    if dist[cur] == cd and sati[cur] < cs:
      continue
    if cur == N - 1:
      echo -cs
      quit()
    
    for (to, ad) in graph[cur]:
      let
        nd = cd + ad
        ns = cs - A[to]
      if dist[to] < nd:
        continue
      if dist[to] == nd and sati[to] <= ns:
        continue
      dist[to] = nd
      sati[to] = ns
      hque.push((nd, ns, to))
