...
when isMainModule:
  var N, M, S: int
  (N, M, S) = inputInts()
  var A = inputs().mapIt(it.parseFloat)
  let lim = M.float
  
  proc f(right: int, rem: float): seq[float] =
    if right == 0:
      return result
    var accA = newSeq[float](right)
    accA[right - 1] = A[right - 1]
    for i in countdown(right - 2, 0):
      accA[i] = accA[i + 1] + A[i]
    
    var mx: float
    var idx: int
    for i in 0..<right:
      if accA[i]/(right - i).float > mx:
        mx = accA[i]/(right - i).float
        idx = i
    
    if rem/(right - idx).float <= lim:
      result = concat(newSeq[float](idx), newSeqWith(right - idx, rem/(right - idx).float))
    else:
      result = concat(f(idx, rem - lim*(right - idx).float), newSeqWith(right - idx, lim))
