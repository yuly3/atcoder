...
when isMainModule:
  var N, M: int
  (N, M) = inputInts()
  var graph: array[601, seq[int]]
  var si, ti: int
  for _ in 0..<M:
    (si, ti) = inputInts().mapIt(it - 1)
    graph[si].add(ti)
  
  var ans = N.float
  for blc in 0..N:
    if blc < N and graph[blc].len < 2:
      continue
    
    var dp: array[601, float]
    for fr in countdown(N - 2, 0):
      var cnt = graph[fr].len
      if blc == fr:
        cnt.dec
      
      var maxExp: float
      for to in graph[fr]:
        let exp = (dp[to] + 1.0) / cnt.float
        dp[fr] += exp
        maxExp.chmax(exp)
      if blc == fr:
        dp[fr] -= maxExp
