...
when isMainModule:
  ...
  var pow2: array[300010, int]
  pow2[0] = 1
  for i in 1..N:
    pow2[i] = pow2[i - 1]*2 mod MOD
  
  var a2idx = collect(initTable):
    for i, ai in sorted(A.toHashSet().toSeq()): {ai: i}
  var initSeq = newSeq[int](a2idx.len)
  for i, ai in A:
    initSeq[a2idx[ai]] += pow2[i]
    if initSeq[a2idx[ai]] >= MOD: initSeq[a2idx[ai]] -= MOD
  var segt = initSegTree(initSeq, (a: int, b: int) => (a + b) mod MOD, () => 0)
  
  var ans = 0
  for i, ai in A:
    segt[a2idx[ai]] = segt[a2idx[ai]] - pow2[i]
    let sm = segt[a2idx[ai]..<a2idx.len]
    ans += sm*modInv(pow2[i + 1], MOD)
    ans %= MOD
  echo ans
