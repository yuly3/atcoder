...
when isMainModule:
  var
    N = inputInt()
    AB = newSeqWith(N, (let ab = inputInts(); (a:ab[0], b:ab[1])))
  
  var uni = initHashSet[int]()
  for (ai, bi) in AB:
    uni.incl(ai)
    uni.incl(ai + bi)
  
  var idx2d = sorted(uni.toSeq())
  var d2idx = collect(initTable):
    for i, di in idx2d: {di: i}
  
  var imos: array[500000, int]
  for (ai, bi) in AB:
    imos[d2idx[ai]] += 1
    imos[d2idx[ai + bi]] -= 1
  for i in 1..<idx2d.len:
    imos[i] += imos[i - 1]
  
  var ans: array[200010, int]
  for i in 0..<idx2d.len - 1:
    var ad = idx2d[i + 1] - idx2d[i]
    ans[imos[i]] += ad
  echo ans[1..N].join(" ")
