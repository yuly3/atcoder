...
when isMainModule:
  ...
  var N, M: int
  (N, M) = inputInts()
  var a, b: array[300, int]
  for i in 0..<M:
    (a[i], b[i]) = inputInts().mapIt(it - 1)
  
  var subgraph: array[200000, int]
  subgraph.fill(1)
  for S in 0..<1 shl N:
    for i in 0..<M:
      if ((S shr a[i]) and (S shr b[i]) and 1) == 1:
        subgraph[S] *= 2
        subgraph[S] %= MOD
  
  let mask = (1 shl N) - 1
  var concatSubgraph: array[200000, int]
  var ans: array[18, int]
  for S in countup(1, 2^N - 1, 2):
    concatSubgraph[S] = subgraph[S]
    var T = (S - 1) and S
    while T > 0:
      if (T and 1) == 1:
        concatSubgraph[S] -= concatSubgraph[T]*subgraph[S and (not T) and mask]
        concatSubgraph[S] %= MOD
      T.dec
      T &= S
    
    var cnt = concatSubgraph[S]*subgraph[mask and (not S)]
    cnt %= MOD
    for i in 1..<N:
      if ((S shr i) and 1) == 1:
        ans[i] += cnt
