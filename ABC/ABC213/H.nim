...
when not declared ATCODER_INTERNAL_MATH_HPP:
  ...
  {.emit: """inline unsigned long long calc_mul(const unsigned long long &a, const unsigned long long &b){return (unsigned long long)(((unsigned __int128)(a)*b) >> 64);}""".}
...
when isMainModule:
  ...
  var N, M, T: int
  (N, M, T) = inputInts()
  var p: array[11, array[11, seq[int]]]
  var ab: seq[(int, int)]
  var ai, bi: int
  for _ in 0..<M:
    (ai, bi) = inputInts().mapIt(it - 1)
    p[ai][bi] = concat(@[0], inputInts())
    ab.add((ai, bi))
  
  var dp: array[11, array[40001, int]]
  dp[0][0] = 1
  
  proc onlineConvolution(left, right: int) =
    if left + 1 == right:
      return
    let mid = (left + right) div 2
    onlineConvolution(left, mid)
    for (a, b) in ab:
      let
        dpa = dp[a][left..<mid]
        pab = p[a][b][0..<right - left]
        ra = convolution(dpa, pab)
      for i in mid..<right:
        dp[b][i] += ra[i - left]
        dp[b][i] %= MOD
      let
        dpb = dp[b][left..<mid]
        rb = convolution(dpb, pab)
      for i in mid..<right:
        dp[a][i] += rb[i - left]
        dp[a][i] %= MOD
    onlineConvolution(mid, right)
  
  onlineConvolution(0, T + 1)
  echo dp[0][T]
