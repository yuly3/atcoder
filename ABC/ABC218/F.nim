...
when isMainModule:
  ...
  (N, M) = inputInts()
  var si, ti: int
  var st = collect(newSeq):
    for _ in 0..<M: (si, ti) = inputInts().mapIt(it - 1); (si, ti)
  
  var graph: array[401, seq[int]]
  for (si, ti) in st:
    graph[si].add(ti)
  
  var st2idx: Table[(int, int), int]
  for i, sti in st:
    st2idx[sti] = i
  
  var dist: array[401, int]
  const INF = 10^9
  dist.fill(INF)
  dist[0] = 0
  
  var spath: HashSet[int]
  var que = initDeque[(int, seq[int])]()
  que.addLast((0, newSeq[int]()))
  while que.len > 0:
    let (cur, path) = que.popFirst()
    if cur == N - 1:
      spath = path.toHashSet()
      break
    for to in graph[cur]:
      if dist[to] != INF:
        continue
      dist[to] = dist[cur] + 1
      que.addLast((to, concat(path, @[st2idx[(cur, to)]])))
  
  let s2n = dist[N - 1]
  if s2n == INF:
    echo newSeqWith(M, -1).join("\n")
    quit()
