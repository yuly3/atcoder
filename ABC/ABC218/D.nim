...
when isMainModule:
  ...
  var point: Table[int, HashSet[int]]
  for (xi, yi) in xy:
    point.mgetOrPut(xi, initHashSet[int]()).incl(yi)
  
  # xy.sort()
  var ans = 0
  for i, (xi, yi) in xy[0..N - 2]:
    for (xj, yj) in xy[i + 1..<N]:
      if xi == xj or yi == yj:
        continue
      if yj in point[xi] and yi in point[xj]:
        ans.inc
  echo ans div 2
