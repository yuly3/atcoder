...
when isMainModule:
  ...
  var idx2a = sorted(toSeq(A.toHashSet()))
  var a2idx = collect(initTable):
    for i, ai in idx2a: {ai: i}
  
  proc op(x, y: (int, int)): (int, int) = (x[0] + y[0], x[1] + y[1])
  var segt = initSegTree(N, op, () => (0, 0))
  
  proc dfs(cur, par, cnt: int): int =
    let (pv, pc) = segt[a2idx[A[cur]]]
    segt[a2idx[A[cur]]] = (pv + A[cur], pc + 1)
    var isLeaf = true
    var (mi, ma) = (10^9, 0)
    for to in graph[cur]:
      if to == par:
        continue
      isLeaf = false
      let ret = dfs(to, cur, cnt + 1)
      mi.chmin(ret)
      ma.chmax(ret)
    
    if isLeaf:
      var idx = segt.max_right(0, (a: (int, int)) => a[1] <= cnt div 2)
      result = idx2a[idx]
      if bitand(cnt, 1) == 0:
        idx = segt.max_right(0, (a: (int, int)) => a[1] < cnt div 2)
        result = (result + idx2a[idx]) div 2
    else:
      if bitand(cnt, 1) == 1:
        result = ma
      else:
        result = mi
    segt[a2idx[A[cur]]] = (pv, pc)
