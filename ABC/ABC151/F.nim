...
when isMainModule:
  ...
  for _ in 0..<N:
    let xi, yi = nextFloat()
    ps.add(complex(xi, yi))

  proc f(tp: Complex64): float = max(ps.mapIt(abs(it - tp)))

  proc g(tx: float): float =
    var (bot, top) = (0.0, 1e4)
    while top - bot > 1e-7:
      let
        mb = (bot*2 + top)/3
        mt = (bot + top*2)/3
      if f(complex(tx, mb)) < f(complex(tx, mt)):
        top = mt
      else:
        bot = mb
    return f(complex(tx, bot))

  var (left, right) = (0.0, 1e4)
