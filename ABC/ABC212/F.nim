...
when isMainModule:
  ...
  (N, M, Q) = inputInts()
  var
    A, B, S, T: array[100001, int]
    bus = newSeqWith(N + 1, newSeq[tuple[s, idx: int]]())
  for i in 0..<M:
    (A[i], B[i], S[i], T[i]) = inputInts()
    bus[A[i]].add((S[i], i))
  for i in 1..N:
    bus[i].sort()
  
  var dub: array[20, array[100001, int]]
  for i in 0..<M:
    let idx = lowerBound(bus[B[i]], (T[i], -1))
    if idx == bus[B[i]].len:
      dub[0][i] = i
    else:
      dub[0][i] = bus[B[i]][idx].idx
  for i in 0..<19:
    for j in 0..<M:
  ...
  for i in 0..<Q:
    (xi, yi, zi) = inputInts()
    var idx = lowerBound(bus[yi], (xi, -1))
    if idx == bus[yi].len:
      ans[i] = $yi
      continue
    
    var cur = bus[yi][idx].idx
    if zi <= S[cur]:
      ans[i] = $yi
      continue
    
    if zi <= T[cur]:
      ans[i] = fmt"{A[cur]} {B[cur]}"
      continue
    ...
    idx = lowerBound(bus[B[cur]], (T[cur], -1))
    if idx == bus[B[cur]].len:
      ans[i] = $B[cur]
      continue
    
    let nx = bus[B[cur]][idx].idx
    if zi <= S[nx]:
      ans[i] = $B[cur]
      continue
