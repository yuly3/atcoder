...
when isMainModule:
  ...
  P.dec
  var divs = makeDivisors(P)
  divs.sort(order=Descending)
  
  var g = collect(newSeq):
    for d in divs: P div d
  for i in 0..<divs.len - 1:
    for j in i + 1..<divs.len:
      if divs[i] mod divs[j] == 0:
        g[j] -= g[i]
  g.applyIt(it mod MOD)
  
  var ans = 1
  for i, d in divs:
    ans += g[i]*((P div d) mod MOD)
    ans %= MOD
  echo ans
