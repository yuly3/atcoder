...
when isMainModule:
  proc op(v: (int, int), fr, to: int): (int, int) =
    if v[1] == 0:
      return (v[0], 1)
    return (v[0] + 1, 0)
  proc merge(v1, v2: (int, int)): (int, int) = (v1[0] + v2[0], v1[1] + v2[1])
  var rerooting = initRerootingTree[200001, (int, int)](op, merge, (0, 0))

  var N = inputInt()
  var ui, vi: int
  for _ in 0..<N - 1:
    (ui, vi) = inputInts().mapIt(it - 1)
    rerooting.addEdge(ui, vi)
