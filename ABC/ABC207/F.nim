...
when isMainModule:
  ...
  var
    dp: array[2002, seq[array[3, int]]]
    reached: array[2001, bool]
  ...
  proc dfs(cur: int): int =
    ...
    for to in graph[cur]:
      ...
      reached[to] = true
      let cnt = dfs(to)
      var nxt = newSeqWith(result + cnt + 2, [0, 0, 0])
      for i in 0..result:
        for j in 0..cnt:
          nxt[i + j][0] += dp[cur][i][0]*dp[to][j][0]
          nxt[i + j][0] %= MOD
          nxt[i + j + 1][0] += dp[cur][i][0]*dp[to][j][1]
          nxt[i + j + 1][0] %= MOD
          nxt[i + j][0] += dp[cur][i][0]*dp[to][j][2]
          nxt[i + j][0] %= MOD
          nxt[i + j + 1][2] += dp[cur][i][1]*dp[to][j][0]
          nxt[i + j + 1][2] %= MOD
          nxt[i + j][1] += dp[cur][i][1]*dp[to][j][1]
          nxt[i + j][1] %= MOD
          nxt[i + j][1] += dp[cur][i][1]*dp[to][j][2]
          nxt[i + j][1] %= MOD
          nxt[i + j][2] += dp[cur][i][2]*dp[to][j][0]
          nxt[i + j][2] %= MOD
          nxt[i + j][2] += dp[cur][i][2]*dp[to][j][1]
          nxt[i + j][2] %= MOD
          nxt[i + j][2] += dp[cur][i][2]*dp[to][j][2]
          nxt[i + j][2] %= MOD
      discard nxt.pop()
      dp[cur] = nxt
      result += cnt
