...
when isMainModule:
  ...
  var lazy, val: array[200001, int]
  for i in 1..N:
    val[i - 1] = i
  
  proc propagate(idx: int): int =
    if graph[idx].len == 0:
      return val[idx]
    result = max(val[idx], max(graph[idx].mapIt(lazy[it])))
  
  const msk = 1 shl 30
  for i, xi in x:
    let nv = (i + 1)*msk + bitand(propagate(xi), msk - 1)
    val[xi] = nv
    lazy[xi] = nv
    for to in graph[xi]:
      val[to] = nv
  ...
  let ans = collect(newSeq):
    for i in 0..<N: bitand(propagate(i), msk - 1)
  echo ans.join(" ")
