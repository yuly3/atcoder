...
when isMainModule:
  ...
  var
    N = inputInt()
    ui, vi, wi: int
    graph: array[200001, seq[(int, int)]]
  for _ in 0..<N - 1:
    (ui, vi, wi) = inputInts()
    ui.dec; vi.dec
    graph[ui].add((vi, wi))
  ...
  while deq.len > 0:
    ...
    for (to, w) in graph[cur]:
      if dist[to] != -1:
        continue
      dist[to] = bitxor(dist[cur], w)
      deq.addLast(to)
  
  var counter: array[60, array[2, int]]
  for i in 0..<N:
    for j in 0..<60:
      counter[j][bitand(dist[i] shr j, 1)].inc
  
  var ans = 0
  for i in 0..<60:
    ans += (counter[i][0]*counter[i][1] mod MOD)*modPow(2, i) mod MOD
    ans %= MOD
  echo ans
