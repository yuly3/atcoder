...
when isMainModule:
  var
    N = inputInt()
    P = inputInts()
    A, B, C: array[200001, int]
  for i in 0..<N:
    (A[i], B[i], C[i]) = inputInts()
  
  var pos: array[200001, int]
  for i, pi in P:
    pos[pi] = i + 1
  
  var accA, accB, accC: array[200001, int]
  for i in 0..<N:
    accA[i + 1] = accA[i] + A[i]
    accB[i + 1] = accB[i] + min(A[i], B[i])
    accC[i + 1] = accC[i] + min(A[i], C[i])

  const INF = 10^18
  var
    dp: array[200001, int]
    segTree = initSegTree(N + 1, (a: int, b: int) => min(a, b), () => INF)
    ans = INF
  dp.fill(INF)
  for i in 1..N:
    dp[i] = min(accB[i - 1], segTree[0..<pos[i]] + accA[i - 1])
    ans.chmin(dp[i] + accC[N] - accC[i])
    segTree[pos[i]] = dp[i] - accA[i]
  echo ans
