...
when isMainModule:
  ...
  var dp: array[100001, array[13, int]]
  dp[0][0] = 1
  for i, si in S:
    for j in 0..12:
      if si == '?':
        for k in 0..9:
          dp[i + 1][(10*j + k) mod 13] += dp[i][j]
          dp[i + 1][(10*j + k) mod 13] %= MOD
      else:
        dp[i + 1][(10*j + ord(si) - ord('0')) mod 13] += dp[i][j]
        dp[i + 1][(10*j + ord(si) - ord('0')) mod 13] %= MOD
  echo dp[len(S)][5]
