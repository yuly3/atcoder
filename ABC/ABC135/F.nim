...
when isMainModule:
  ...
  var z = t & s.repeat(1 + (s.len + t.len - 1) div s.len)
  var lcp = z_algorithm(z)[t.len..^1]
  
  var graph: array[4000010, seq[int]]
  for i in 0..<lcp.len:
    if lcp[i] >= t.len:
      graph[i].add((i + t.len) mod s.len)
  
  var ans: array[4000010, int]
  var stat: array[4000010, int8]
