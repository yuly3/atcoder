...
when isMainModule:
  var H, W, N: int
  (H, W, N) = inputInts()
  type coma = tuple[r, c, a, i: int]
  var rca: seq[coma]
  for i in 0..<N:
    rca.add((let line = inputInts(); (line[0], line[1], line[2], i)))

  var uni = initHashSet[int](2^20)
  for (r, c, a, i) in rca:
    uni.incl(a)
  var a2idx = initTable[int, int](2^20)
  for i, ai in sorted(toSeq(uni)):
    a2idx[ai] = i

  var groupByA: array[200001, seq[tuple[r, c, i: int]]]
  for (r, c, a, i) in rca:
    groupByA[a2idx[a]].add((r, c, i))

  var rmax, cmax: array[200001, int]
  rmax.fill(-1)
  cmax.fill(-1)
  var ans = newSeq[int](N)
  for j in countdown(a2idx.len - 1, 0):

    var done: seq[tuple[r, c, v: int]]
    for (r, c, i) in groupByA[j]:
      let v = max(rmax[r], cmax[c]) + 1
      ans[i] = v
      done.add((r, c, v))
