...
when isMainModule:
  var M = inputInt()
  var graph: array[10, seq[int8]]
  var ui, vi: int8
  for _ in 0..<M:
    (ui, vi) = inputInts().mapIt(it.int8)
    graph[ui].add(vi)
    graph[vi].add(ui)
  var s: array[8, int8]
  for i, si in inputInts():
    s[i] = si.int8
  ...
  var reach = initHashSet[array[8, int8]](2^21)
  reach.incl(s)
  var que = initSimpleQueue[(array[8, int8], int8, int32)]()
  que.push((s, emp, 0.int32))
  const g = [1, 2, 3, 4, 5, 6, 7, 8].mapIt(it.int8)

  block bfs:
    while not que.empty:
      var (cur, to, dist) = que.front(); que.pop()
      if cur == g:
        echo dist
        break bfs

      var fr2idx: array[10, int8]
      for i, j in cur:
        fr2idx[j] = i.int8
