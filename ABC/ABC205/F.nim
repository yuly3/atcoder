...
when isMainModule:
  ...
  var mfg = initMFGraph[int](H + W + 2*N + 2)
  let
    S = H + W + 2*N
    T = H + W + 2*N + 1
  ...
  var ai, bi, ci, di: int
  for i in 0..<N:
    (ai, bi, ci, di) = inputInts().mapIt(it - 1)
    
    for r in ai..ci:
      mfg.addEdge(r, H + W + i, 1)
    mfg.addEdge(H + W + i, H + W + N + i, 1)
    for c in bi..di:
      mfg.addEdge(H + W + N + i, H + c, 1)
  
  for c in 0..<W:
    mfg.addEdge(H + c, T, 1)
  
  let ans = mfg.flow(S, T)
  echo ans
