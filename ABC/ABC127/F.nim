  ...
  for _ in 0..<Q:
    ...
    if query[0] == 1:
      let
        ...
        b = query[2]
      uniA.incl(a)
      queries.add((true, a, b))
    else:
      queries.add((false, 0, 0))
  
  let
    idx2a = sorted(toSeq(uniA))
    a2idx = collect(initTable):
      for i, ai in idx2a: {ai: i}
    N = idx2a.len
  
  var
    valSegt = initSegTree(N, (x: int, y: int) => x + y, () => 0)
    cntSegt = initSegTree(N, (x: int, y: int) => x + y, () => 0)
    sb, cnt: int
    ans: seq[string]
  for (cmd, a, b) in queries:
    if cmd:
      valSegt[a2idx[a]] = valSegt[a2idx[a]] + a
      cntSegt[a2idx[a]] = cntSegt[a2idx[a]] + 1
      cnt.inc
      sb += b
    else:
      let
        idx = cntSegt.max_right(0, (x: int) => x < (cnt + 1) div 2)
        med = idx2a[idx]
        ovsm = valSegt[idx..<N]
        ovcn = cntSegt[idx..<N]
        unsm = valSegt.all_prod() - ovsm
        uncn = cntSegt.all_prod() - ovcn
        fAns = abs(ovcn*med - ovsm) + abs(uncn*med - unsm) + sb
      ans.add(fmt"{med} {fAns}")
  echo ans.join("\n")
