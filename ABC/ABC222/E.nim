...
when isMainModule:
  ...
  (N, M, K) = inputInts()
  var
    A = inputInts().mapIt(it - 1)
    graph: array[1001, seq[int]]
    edges = initTable[(int, int), int]()
    ui, vi: int
  for i in 0..<N - 1:
  ...
  var counter1, counter2: array[1001, int]
  var cmd: int
  
  proc dfs(cur, par: int) =
    if cur == A[cmd]:
      for i in 0..<N - 1:
    ...
    for to in graph[cur]:
      ...
      if edge notin edges:
        edge = (to, cur)
      counter2[edges[edge]].inc
      dfs(to, cur)
      counter2[edges[edge]].dec
  
  for i in 0..<M - 1:
    cmd.inc
    dfs(A[i], -1)
  
  var dp: array[100010, int]
  dp[0] = 1
  var sm = 0
  for i in 0..<N - 1:
    var ndp: array[100010, int]
    for j in 0..sm:
      ndp[j] += dp[j]
      if ndp[j] >= MOD: ndp[j] -= MOD
      ndp[j + counter1[i]] += dp[j]
      if ndp[j + counter1[i]] >= MOD: ndp[j + counter1[i]] -= MOD
    sm += counter1[i]
    dp = ndp
  
  if K + sm < 0 or (K + sm) mod 2 == 1:
    echo 0
  else:
    echo dp[(K + sm) div 2]
