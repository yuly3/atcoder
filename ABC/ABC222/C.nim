...
when isMainModule:
  var N, M: int
  (N, M) = inputInts()
  var A = newSeqWith(2*N, input())
  
  var member = collect(newSeq):
    for i in 0..<2*N: (0, i)
  
  const leftWin = [('G', 'C'), ('C', 'P'), ('P', 'G')]
  for j in 0..<M:
    var nMember: seq[(int, int)]
    for i in 0..<N:
      let
        (win1, num1) = member[2*i]
        (win2, num2) = member[2*i + 1]
      if (A[num1][j], A[num2][j]) in leftWin:
        nMember.add((win1 - 1, num1))
        nMember.add((win2, num2))
      elif A[num1][j] == A[num2][j]:
        nMember.add((win1, num1))
        nMember.add((win2, num2))
      else:
        nMember.add((win1, num1))
        nMember.add((win2 - 1, num2))
    nMember.sort()
    member = nMember
