...
when isMainModule:
  var N, K: int
  (N, K) = inputInts()
  var A: array[801, array[801, int]]
  for i in 0..<N:
    let ai = inputInts()
    for j in 0..<N:
      A[i][j] = ai[j]

  var sa = initHashSet[int]()
  for i in 0..<N:
    for j in 0..<N:
      sa.incl(A[i][j])
  var
    idxToA = sorted(toSeq(sa))
    aToIdx = collect(initTable):
      for i, ai in idxToA: {ai: i}
  ...
  proc f(t: int): bool =
    var s = newSeqWith(N + 1, newSeq[int](N + 1))
    for i in 0..<N:
      for j in 0..<N:
        if A[i][j] <= t:
          s[i + 1][j + 1] = 1
    ...
    for i in K..N:
      for j in K..N:
        if s[i][j] - s[i - K][j] - s[i][j - K] + s[i - K][j - K] >= c:
          return true
    return false
  
  var (ok, ng) = (idxToA.len - 1, -1)
  while ok - ng > 1:
    let mid = (ok + ng) div 2
    if f(mid):
      ok = mid
