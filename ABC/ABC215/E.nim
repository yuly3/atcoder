...
when isMainModule:
  ...
  var uniS = collect(initHashSet):
    for si in S: {si}
  var s2idx = collect(initTable):
    for i, si in sorted(toSeq(uniS)): {si: i}
  var cnt = s2idx.len + 1
  
  var dp: array[1 shl 11, array[11, int]]
  dp[0][0] = 1
  for i in 0..<N:
    let j = s2idx[S[i]] + 1
    var ndp: array[1 shl 11, array[11, int]]
    for ps in 0..<1 shl cnt:
      for lst in 0..<cnt:
        ndp[ps][lst] += dp[ps][lst]
        ndp[ps][lst] %= MOD
        if bitand(ps shr j, 1) == 1 and lst != j:
          continue
        ndp[ps or (1 shl j)][j] += dp[ps][lst]
        ndp[ps or (1 shl j)][j] %= MOD
    dp = ndp
  
  var ans: int
  for s in 1..<1 shl cnt:
    ans += sum(dp[s])
  ans %= MOD
  echo ans
