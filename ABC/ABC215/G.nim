...
when isMainModule:
  ...
  var c2cnt = c.toCountTable()
  var cnt2cnt: Table[int, int]
  for (ci, cnt) in c2cnt.pairs():
    cnt2cnt.mgetOrPut(cnt, 0).inc
  
  var cmb = initCombination(N, MOD)
  var ans: seq[int]
  for k in 1..N:
    var exp: int
    for (x, cnt) in cnt2cnt.pairs():
      exp += cnt*(cmb.nCr(N, k) - cmb.nCr(N - x, k))
    exp %= MOD
    exp *= modInv(cmb.nCr(N, k), MOD)
    exp %= MOD
    ans.add(exp)
  echo ans.join("\n")
